...
```python
app = Starlette(
    middleware=[Middleware(mcp_auth.bearer_auth_middleware(
        "jwt",
...
async def custom_verification(token: str):
...
```ts
import { MCPAuthJwtVerificationError } from 'mcp-auth';
import express from 'express';

const app = express();
app.use(
  '/mcp',
  mcpAuth.bearerAuth(async (token) => {
    // Implement your custom verification logic here
