...
MCP Auth verifies JWT access tokens locally against your authorization server's JWKS — no extra network call per request. The verified token claims (such as `sub`, the user's ID) are what the `whoami` tool will return.
