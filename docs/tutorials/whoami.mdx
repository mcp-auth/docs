...
Logto supports the [OpenID Connect Discovery](https://openid.net/specs/openid-connect-discovery-1_0.html) standard, so you can use the built-in function to fetch the metadata and initialize the MCP Auth instance.
...
auth_issuer = '<logto-issuer-endpoint>'  # Replace with your Logto issuer endpoint
...
def verify_access_token(token: str) -> AuthInfo:
    ...
    endpoint = auth_server_config.metadata.userinfo_endpoint # Logto supports the userinfo endpoint
    if not endpoint:
        raise ValueError(
...
const authIssuer = '<logto-issuer-endpoint>'; // Replace with your Logto issuer endpoint
...
:::info
Check out the [MCP Auth Python SDK repository](https://github.com/mcp-auth/python/blob/master/samples/server/whoami.py) for the complete code of the MCP server.
:::
...
:::info
Check out the [MCP Auth Node.js SDK repository](https://github.com/mcp-auth/js/blob/master/packages/sample-servers/src/whoami.ts) for the complete code of the MCP server.
:::
...
- Using JWT (JSON Web Token) mode for Bearer authentication
- Leveraging resource indicators (RFC 8707) to specify the resources being accessed
- Implementing custom access control mechanisms, such as role-based access control (RBAC)
