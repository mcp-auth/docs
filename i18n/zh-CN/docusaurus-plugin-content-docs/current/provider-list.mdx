...
| Provider                                                  | 类型           | OAuth 2.1 | Metadata URL | 动态客户端注册 | 资源指示器 (Resource Indicator)[^1] |
| --------------------------------------------------------- | -------------- | --------- | ------------ | --------------------------- | ---------------------- |
...
[^1]: 资源指示器 (Resource Indicator) 代表 [RFC 8707: OAuth 2.0 的资源指示器](https://datatracker.ietf.org/doc/html/rfc8707)，这是一个用于指示客户端希望访问的资源的标准。

[^2]: Logto 正在开发对动态客户端注册的支持。

[^3]: 虽然 Keycloak 支持动态客户端注册，但其客户端注册端点不支持 CORS，导致大多数 MCP 客户端无法直接注册。

[^4]: Auth0 和 Descope 支持多资源刷新令牌 (MRRT)，但不完全支持 RFC 8707。资源指示器 (Resource Indicator) 的支持有限，且不符合标准。
...
2. **如果你正在开发将被公共应用（MCP 客户端）使用的 MCP 服务器**：
   ...
   2. 或者，你可以开发自定义注册流程，让 MCP 客户端通过安全可控的流程（如 Web 界面或你控制的 API 端点）在连接器 (Connector) 处注册，无需依赖动态客户端注册。
