...
[^1]: 資源標示符 (Resource Indicator) 代表 [RFC 8707: Resource Indicators for OAuth 2.0](https://datatracker.ietf.org/doc/html/rfc8707)，這是一個用於標示客戶端欲存取資源的標準。

[^2]: Logto 正在開發動態客戶端註冊（Dynamic Client Registration）支援。

[^3]: 雖然 Keycloak 支援動態客戶端註冊，但其註冊端點不支援 CORS，導致大多數 MCP 客戶端無法直接註冊。

[^4]: Auth0 與 Descope 支援多資源重新整理權杖（MRRT, Multi-resource refresh tokens），但不完全符合 RFC 8707。資源標示符支援有限且非標準化。

## 動態客戶端註冊（Dynamic Client Registration）是必需的嗎？ \{#is-dcr-required}

[動態客戶端註冊 (Dynamic Client Registration)](https://datatracker.ietf.org/doc/html/rfc7591) 並非 MCP 伺服器與 MCP Auth 的必要條件。實際上，你可以根據需求選擇最適合的方式：
...
2. **如果你正在開發將由公開應用程式（MCP 客戶端）使用的 MCP 伺服器**：
   ...
   2. 或者，你可以開發自訂註冊流程，讓 MCP 客戶端透過你控制的安全流程（如網頁介面或 API 端點）向連接器註冊，而不依賴動態客戶端註冊。
      只要連接器支援 Management API 或類似功能，你就可以在自訂端點中用來註冊 MCP 客戶端。

## 測試你的連接器 \{#test-your-provider}
